...
public class GameGui extends Screen implements IViewRootDefaultImpl {
    ...
    /*
     * NeoForge port: Kirin injects IViewRootDefaultImpl into Screen through
     * loom's interface injection plus MixinScreen. Neither exists here, so
     * GameGui implements the interface itself.
     */
    private final Bounds screenBounds = new Bounds(0, 0, 0, 0);
    private final Padding contentPadding = new Padding(0, 0, 0, 0);
    ...
    @Override
    public List<NarratableEntry> buttons() {
        return narratables;
    }
    ...
    public void render(GuiGraphics context, int mouseX, int mouseY, float partialTicks) {
        // NeoForge port: MixinScreen used to keep the screen bounds in sync at
        // the end of Screen.init(II). ScrollContainer manages its own bounds
        // and does not call super here, so this only affects real screens.
